...
import (
	...
	"github.com/triton-inference-server/triton_distributed/deploy/compoundai/internal/envoy"
	...
	"github.com/triton-inference-server/triton_distributed/deploy/compoundai/pkg/compoundai/system"
	...
	compounadaiConversion "github.com/triton-inference-server/triton_distributed/deploy/compoundai/api/compoundai/conversion"
)
...
func (r *CompoundAINimDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	if yataiClient != nil && clusterName != nil {
		...
		compoundAINimIsNotFound := err != nil && strings.Contains(err.Error(), "not found")
		if err != nil && !compoundAINimIsNotFound {
			return
		}
		if compoundAINimIsNotFound {
...
monitoring.options.insecure=true`
	...
	args = append(args, "./env/docker/entrypoint.sh", "start-http-server")
	...
	if len(opt.compoundAINimDeployment.Spec.ExternalServices) > 0 {
		...
		for _, key := range keys {
			...
			if service.DeploymentSelectorKey != "name" {
				return nil, errors.Errorf("DeploymentSelectorKey '%s' not supported. Only 'name' is supported", service.DeploymentSelectorKey)
			}
