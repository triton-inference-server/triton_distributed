...
def run_publishers(processes, publisher_count, event_type=None):
    ...
    for i in range(publisher_count):
        log_file = tempfile.NamedTemporaryFile(delete=False, mode="w", prefix=f"publisher_{i + 1}_",
                                               suffix=".log")
        publisher_args = [sys.executable, "publisher.py", "--publisher_id", str(i + 1)]
        if event_type:
            publisher_args.extend(["--event_type", event_type])
        proc = subprocess.Popen(publisher_args, stdout=log_file,
                                stderr=log_file)
...
def run_subscribers(processes, subscriber_count, event_type=None):
    ...
    for i in range(subscriber_count):
        log_file = tempfile.NamedTemporaryFile(delete=False, mode="w", prefix=f"subscriber_{i + 1}_",
                                               suffix=".log")
        subscriber_args = [sys.executable, "subscriber.py", "--subscriber_id", str(i + 1), "--channel", "publisher"]
        if event_type:
            subscriber_args.extend(["--event_type", event_type])
        proc = subprocess.Popen(subscriber_args,
                                stdout=log_file,
                                stderr=log_file)
        processes.append(proc)
