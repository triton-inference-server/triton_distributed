...
function test_helm_chart([object] $config) {
  ...
  if ($fail_count -gt 0) {
    write-minimal "Failed: ${fail_count} (${total_fail_checks}), Passed: ${pass_count} ($total_pass_checks), Tests: $($config.tests.count) ($($total_fail_checks + $total_pass_checks))" 'Red'
    return $false
  }
  else
  {
    write-minimal "Passed: ${pass_count} (${total_pass_checks}), Tests: $($config.tests.count) ($($total_fail_checks + $total_pass_checks))" 'Green'
    return $true
  }
