...
pub struct ChatCompletionChoice {
    ...
    pub finish_reason: NvidiaFinishReason,
...
pub struct ChatCompletionChoiceDelta {
    ...
    /// Only `stop` and `length` are currently supported by NIM.
    /// NIM may also provide additional reasons in the future, such as `error`, `timeout` or `cancelation`.
    pub finish_reason: Option<NvidiaFinishReason>,
...
pub enum NvidiaFinishReason {
    OpenAI(OpenAIFinishReason),
    Cancelled,
    Null,
}

impl Serialize for NvidiaFinishReason {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match self {
            NvidiaFinishReason::OpenAI(reason) => reason.serialize(serializer),
            NvidiaFinishReason::Cancelled => serializer.serialize_str("cancelled"),
            NvidiaFinishReason::Null => serializer.serialize_str("null"),
        }
    }
}

impl<'de> Deserialize<'de> for NvidiaFinishReason {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct NvidiaFinishReasonVisitor;

        impl Visitor<'_> for NvidiaFinishReasonVisitor {
            type Value = NvidiaFinishReason;

            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("a valid FinishReason string")
            }
            ...
            {
                match value {
                    "cancelled" => Ok(NvidiaFinishReason::Cancelled),
                    "null" => Ok(NvidiaFinishReason::Null),
                    other => {
                        OpenAIFinishReason::deserialize(de::value::StrDeserializer::<E>::new(other))
                            .map(NvidiaFinishReason::OpenAI)
                            .map_err(|_| de::Error::invalid_value(Unexpected::Str(other), &self))
                    }
                }
            }
        }

        deserializer.deserialize_str(NvidiaFinishReasonVisitor)
    }
}

/// from_str trait
impl std::str::FromStr for NvidiaFinishReason {
    type Err = String;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "cancelled" => Ok(NvidiaFinishReason::Cancelled),
            "null" => Ok(NvidiaFinishReason::Null),
            other => serde_json::from_str::<OpenAIFinishReason>(&format!("\"{}\"", other))
                .map(NvidiaFinishReason::OpenAI)
                .map_err(|_| format!("Unknown FinishReason: {}", other)),
        }
    }
}

impl fmt::Display for NvidiaFinishReason {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            NvidiaFinishReason::OpenAI(reason) => write!(f, "{:?}", reason), // Note: Uses Debug output
            NvidiaFinishReason::Cancelled => write!(f, "cancelled"),
            NvidiaFinishReason::Null => write!(f, "null"),
        }
    }
}
...
impl TryFrom<common::StreamingCompletionResponse> for ChatCompletionChoice {
    ...
    fn try_from(response: common::StreamingCompletionResponse) -> Result<Self, Self::Error> {
        let finish_reason = match &response.delta.finish_reason {
            ...
            Some(common::FinishReason::ToolCalls) => {
                NvidiaFinishReason::OpenAI(async_openai::types::FinishReason::ToolCalls)
            }
            Some(common::FinishReason::ContentFilter) => {
                NvidiaFinishReason::OpenAI(async_openai::types::FinishReason::ContentFilter)
            }
            Some(common::FinishReason::Error(err_msg)) => {
                return Err(anyhow::anyhow!("finish_reason::error = {}", err_msg));
            }
            Some(common::FinishReason::Cancelled) => NvidiaFinishReason::Cancelled,
            None => NvidiaFinishReason::Null,
        };
...
impl TryFrom<common::StreamingCompletionResponse> for ChatCompletionChoiceDelta {
    ...
    fn try_from(response: common::StreamingCompletionResponse) -> Result<Self, Self::Error> {
        let finish_reason = match &response.delta.finish_reason {
            ...
            None => None,
            _ => {
                tracing::log::warn!("Unknown finish reason: {:?}", response.delta.finish_reason);
