...
PYTORCH_CONFIG_KEYS = {
    "use_cuda_graph", "cuda_graph_batch_sizes",
    "cuda_graph_max_batch_size", "cuda_graph_padding_enabled",
    "enable_overlap_scheduler", "kv_cache_dtype", "torch_compile_enabled",
    "torch_compile_fullgraph", "torch_compile_inductor_enabled"
}

LLM_ENGINE_KEYS = {
    "model", "tokenizer", "tokenizer_model", "skip_tokenizer_init",
    "trust_remote_code", "tensor_parallel_size", "dtype", "revision",
    "tokenizer_revision", "speculative_model", "enable_chunked_prefill"
}
...
def _get_llm_args(args_dict):
    pytorch_config_args = {
        ...
        if k in PYTORCH_CONFIG_KEYS and v is not None
    }
    llm_engine_args = {
        k: v
        for k, v in args_dict.items()
        if k in LLM_ENGINE_KEYS and v is not None
    }
...
def _init_engine_args(engine_args_filepath):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to parse engine config: {e}")
...
def parse_tensorrt_llm_args() -> Tuple[Dict[str, Any], Dict[str, Any]]:
     ...
     parser.add_argument("--engine_args", type=str, required=True, help="Path to the engine args file")
