...
class TensorrtLLMEngine:
    ...
    def _init_engine(self):
        ...
        if isinstance(self._llm_engine, Exception):
            e = self._llm_engine
            trtllm_logger.log_error(f"[trtllm] Failed to start engine: {e}")
    ...
    async def _run_llm_engine(self):
        ...
        self._llm_engine = None
        trtllm_logger.log_info("[trtllm] Shutdown complete")


    @triton_endpoint(Request, Response)
    async def generate(self, request):
        self._ongoing_request_count += 1
        trtllm_logger.debug(f"Received request: {request}")
        ...
        async for response in self._llm_engine.generate_async(
            ...
            trtllm_logger.debug(f"Generated response: {response}")
...
async def worker(runtime: DistributedRuntime, engine_args: AsyncEngineArgs):
    ...
    await endpoint.serve_endpoint(TensorrtLLMEngine().generate)
